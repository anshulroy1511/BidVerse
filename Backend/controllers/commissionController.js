...
export const proofOfCommission = catchAsyncErrors(async(req,res,next) => {
    ...
    if(user.unpaidCommission === 0){
        return res.status(200).json({
            success: true,
            message: "You do not have any unpaid commission" ,
        });
    }
    ...
    if(user.unpaidCommission < amount){
        return next(
            new ErrorHandler(
                `The amount exceeds your unpaid commission balance. Please enter am amount up to ${user.unpaidCommission}`, 
                403 
            )
        );
    }
    ...
    res.status(201).json({
        success: true,
        message:
        "Your proof has been submitted successfully. We will review it and respomd to you within 24 hours.",
        commissionProof,
    });
});